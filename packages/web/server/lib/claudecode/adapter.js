...
function createApp(cwd) {
  ...
  app.post('/session', async (req, res) => {
    ...
    const requestDirectory = typeof req.query.directory === 'string' && req.query.directory.trim().length > 0
  ...
  app.post('/session/:id/prompt_async', async (req, res) => {
    ...
    if (!Object.hasOwn(sessions, id)) {
      const promptDirectory = typeof req.query.directory === 'string' && req.query.directory.trim().length > 0
        ? req.query.directory.trim()
        : cwd;
      const now = new Date().toISOString();
      sessions[id] = {
